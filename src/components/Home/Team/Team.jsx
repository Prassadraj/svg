    ...
    >
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-2 laptop:grid-cols-3 gap-10 place-items-center">
        ...
        {data.map((val, i) => (
          <div
            key={i}
            className={`relative rounded-xl shadow-sm shadow-gray-400 w-[300px] h-[380px] bg-gradient-to-t from-[#503B31] to-[#B67E70] justify-end pb-6 items-center flex flex-col gap-4 group
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <p>{val.name}</p>
              <p className={`${montLight.className} text-sm`}>{val.position}</p>
            </div>
          </div>
        ))}
      </div>
